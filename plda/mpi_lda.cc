...
int main(int argc, char** argv) {
  ...
  LDACorpus corpus;
  set<string> allwords;
  int corpus_size = DistributelyLoadAndInitTrainingCorpus(flags.training_data_file_,
                                     flags.num_topics_,
                                     myid, pnum, &corpus, &allwords);
  CHECK_GT(corpus_size, 0);
  std::cout << "Training data loaded, size = " << corpus_size << std::endl;
